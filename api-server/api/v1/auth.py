...
class LoginRequest(BaseModel):
    identifier: str = Field(min_length=1, max_length=255, validation_alias=AliasChoices("identifier", "email"))
...
async def login(request: Request, response: Response, payload: LoginRequest, db: AsyncSession = Depends(get_db)):
    user = await authenticate_user(payload.identifier, payload.password, db)
    if not user:
    ...
    if user.mfa_enabled and user.mfa_secret:
        await db.execute(delete(MfaChallenge).where(MfaChallenge.expires_at <= datetime.utcnow()))
        ...
        db.add(MfaChallenge(
            ...
            expires_at=datetime.fromtimestamp(challenge_payload["exp"]),
        ))
        await db.flush()
