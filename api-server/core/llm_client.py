...
def pseudonymize_pii(data: Any) -> Any:
    ...
    if isinstance(data, dict):
        new_dict = {}
        medical_keys = {
            "allergies", "antecedents_medicaux", "antecedents_medicaux_enc",
            "contre_indications", "contre_indications_enc", "note_interne",
            "notes_internes", "medical_notes", "dossier_medical",
            "diagnostic", "symptomes", "symptoms", "prescription",
            "effets_secondaires",
        }
        for k, v in data.items():
            key_lower = str(k).lower()
            if key_lower in medical_keys:
                new_dict[k] = "[DONNEE_MEDICALE_REDACTEE]"
            # Masquage par clé
            elif k in ("nom", "name", "full_name", "patient_name"):
    ...
    if isinstance(data, str):
        ...
        data = re.sub(r"(?<!\d)\+?\d[\d\s().-]{7,}\d(?!\d)", "[TEL]", data)
        return data[:12000] + ("…" if len(data) > 12000 else "")
    return data


def _configured_model(settings: Any, provider: str) -> str:
    return str(getattr(settings, f"{provider}_model", "") or "").strip()


def _model_is_allowed(settings: Any, provider: str, model: str, requested: Optional[str]) -> bool:
    configured = _configured_model(settings, provider)
    raw = str(getattr(settings, "llm_model_allowlist", "") or "")
    if raw.strip():
        allowed = {item.strip().lower() for item in raw.split(",") if item.strip()}
        return model.lower() in allowed or f"{provider}:{model}".lower() in allowed
    return not requested or not configured or model == configured
...
class LLMClient:
    ...
    async def chat(
        ...
        *,
        ...
        selected_model = model or _configured_model(self._settings, provider)
        if not selected_model or not _model_is_allowed(self._settings, provider, selected_model, model):
            return LLMUnavailable(provider, "modèle refusé par l'allowlist de production")
        safe_messages = pseudonymize_pii(messages)
        max_tokens = min(
            int(max_tokens),
        ...
        cache_key = make_cache_key(safe_messages, model=selected_model, temperature=temperature,
        ...
        for attempt, delay in enumerate(RETRY_BACKOFF_S, start=1):
            try:
                ...
                resp = LLMResponse(text=text, provider=provider,
                                   model=selected_model, usage=usage, latency_ms=latency_ms)
                if use_cache:
    ...
    async def _call_openai(
        ...
        base_url = str(getattr(self._settings, "openai_base_url", "https://api.openai.com/v1")).rstrip("/")
