...
async def generate_lot_barcode(
    lot_id: int,
    db: AsyncSession,
    clinic_id: Optional[int] = None,
) -> bytes:
...
async def generate_lot_label(
    ...
    qr_bytes = await generate_lot_qr(lot_id, db, clinic_id=clinic_id)
    barcode_bytes = await generate_lot_barcode(lot_id, db, clinic_id=clinic_id)
