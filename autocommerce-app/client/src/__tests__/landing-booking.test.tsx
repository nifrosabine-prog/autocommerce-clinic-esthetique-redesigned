...
describe("LandingPage — Public Gateway et BookingRequest", () => {
  ...
  it("bloque la réservation si la notice de confidentialité n’est pas acceptée", async () => {
    const reserveRdv = vi.spyOn(publicApi, "reserveRdv");
    renderLanding();
    await screen.findByText("Réserver un rendez-vous");
    fireEvent.change(screen.getByLabelText("Prénom"), { target: { value: "Ada" } });
  ...
  it("envoie une réservation publique avec les données sélectionnées", async () => {
    const reserveRdv = vi.spyOn(publicApi, "reserveRdv").mockResolvedValue({
      data: { booking_request_id: 42, statut: "pending", duplicate: false },
    } as never);
    renderLanding();
    await screen.findByText("Réserver un rendez-vous");
    ...
    expect(await screen.findByText(/Référence #42/)).toBeInTheDocument();
    expect(screen.getByText(/La clinique doit encore confirmer ce créneau/)).toBeInTheDocument();
  });
