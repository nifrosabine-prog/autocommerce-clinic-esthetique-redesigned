...
@pytest.mark.asyncio
async def test_manual_invoice_links_pending_dossier(db, patient, medecin):
    dossier = DossierMedical(
        clinic_id=1, patient_id=patient.id, praticien_id=medecin.id,
        date_acte=datetime.utcnow(), actes_details=[{"nom": "Soin visage premium", "prix": 180.0}],
    ...
    facture = await create_facture({
        ...
        "actes": [{"description": "Soin visage premium", "prix": "180.000", "quantite": 1}],
    }, created_by=medecin.id, db=db)

    await db.refresh(dossier)
    assert facture.dossier_id == dossier.id
    assert dossier.statut_facturation == "facture"


@pytest.mark.asyncio
async def test_invoice_generation_for_same_dossier_is_blocked(db, patient, medecin):
    dossier = DossierMedical(
        clinic_id=1, patient_id=patient.id, praticien_id=medecin.id,
        date_acte=datetime.utcnow(), actes_details=[{"nom": "Soin visage premium", "prix": 180.0}],
        statut_facturation="en_attente",
    ...
    db.add(dossier)
    await db.flush()
    first = await create_facture({
        "patient_id": patient.id, "dossier_id": dossier.id,
        "actes": [{"description": "Soin visage premium", "prix": "180.000", "quantite": 1}],
    }, created_by=medecin.id, db=db)
    assert first.dossier_id == dossier.id
