...
# rsync vers un autre serveur ssh). Le fichier est chiffré et authentifié
...
# AES-CBC ne fournit pas d’authentification : ajouter un HMAC séparé.
openssl dgst -sha256 -hmac "$BACKUP_ENCRYPTION_KEY" -r "$BACKUP_FILE" \
  | awk '{print $1}' > "$MAC_FILE"
chmod 600 "$BACKUP_FILE" "$MAC_FILE"

# Rétention
find "$BACKUP_ROOT" -type f \
  \( -name 'autocommerce_*.sql.gz.enc' -o -name 'autocommerce_*.sql.gz.enc.sha256' \) \
  -mtime +"$KEEP_DAYS" -delete

# Vérification intégrité : fichier non vide + HMAC + déchiffrement + gunzip -t
if [[ ! -s "$BACKUP_FILE" || ! -s "$MAC_FILE" ]]; then
  echo "[$(date +%FT%T)] ERREUR : backup vide — ${BACKUP_FILE}" >&2
  exit 2
fi
EXPECTED_MAC="$(cat "$MAC_FILE")"
ACTUAL_MAC="$(openssl dgst -sha256 -hmac "$BACKUP_ENCRYPTION_KEY" -r "$BACKUP_FILE" | awk '{print $1}')"
if [[ -z "$EXPECTED_MAC" || "$EXPECTED_MAC" != "$ACTUAL_MAC" ]]; then
  echo "[$(date +%FT%T)] ERREUR : HMAC backup invalide — ${BACKUP_FILE}" >&2
  exit 3
fi
