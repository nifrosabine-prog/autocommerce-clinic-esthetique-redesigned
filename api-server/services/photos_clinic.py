...
from models.database import Consentement, DossierMedical, Patient
...
async def upload_photo(
    ...
    """
    # Ownership obligatoire avant tout traitement ou persistance.
    ...
    if dossier_id is not None:
        ...
        if not dossier:
            raise ValueError("Dossier médical introuvable pour ce patient")
...
async def get_decrypted_photo(
    ...
    if utilisateur_id is not None:
        await log_access(
            ...
            patient_id=patient_id,
            action="READ_PHOTO_THUMBNAIL" if thumbnail else "READ_PHOTO",
            resource_type="photo",
