...
def _build_simulation_prompt(zone: str, intensite: int, instructions: Optional[str] = None) -> str:
    ...
    base_prompt = (
        ...
        f"corresponding to intensity {intensite}/100."
    )
    
    if instructions:
        base_prompt += f" Follow these specific practitioner instructions: {instructions}."

    base_prompt += (
        " The outcome must look like a conservative, "
...
def _generate_ai_simulation_image(
    ...
    masque_bytes: Optional[bytes] = None
) -> bytes:
    ...
    prompt = _build_simulation_prompt(zone, intensite, instructions)
    ...
    if masque_bytes:
        # S'assurer que le masque est au bon format (PNG RGBA)
        mask_img = PILImage.open(io.BytesIO(masque_bytes)).convert("RGBA")
        # Redimensionner le masque pour correspondre à l'image préparée
        source_img = PILImage.open(prepared_image)
        mask_img = mask_img.resize(source_img.size, PILImage.LANCZOS)
        
        mask_io = io.BytesIO()
        mask_img.save(mask_io, format="PNG")
        mask_io.seek(0)
        files["mask"] = ("mask.png", mask_io.getvalue(), "image/png")
...
async def generer_simulation_ia(
    ...
    # 3. Traitement du masque si présent
    masque_bytes = None
    url_masque = None
    if masque_base64:
        if "," in masque_base64:
            masque_base64 = masque_base64.split(",")[1]
        masque_bytes = base64.b64decode(masque_base64)
        
        # Sauvegarder le masque (chiffré)
        ...
        mask_filename = f"mask_{patient_id}_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.enc"
        url_masque = os.path.join(mask_dir, mask_filename)
        with open(url_masque, "wb") as f:
            f.write(m_nonce + m_ciphertext)

    # 4. Simulation IA réelle, sérialisée pour protéger le VPS et le budget.
    ...
    async with SIMULATION_SEMAPHORE:
        simulated_bytes = await asyncio.to_thread(
            ...
            zone, intensite, instructions, masque_bytes
        )
