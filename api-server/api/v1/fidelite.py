...
async def historique_route(
    ...
    historique = await get_historique(patient_id, db, clinic_id=current_user["clinic_id"])
...
async def gagner_route(
    ...
    try:
        tx = await add_points(
            patient_id, payload.points, payload.motif, db,
    ...
    try:
        ...
        patient_result = await db.execute(sa_select(SAPatient).where(
            SAPatient.id == patient_id,
            SAPatient.clinic_id == current_user["clinic_id"],
        ))
