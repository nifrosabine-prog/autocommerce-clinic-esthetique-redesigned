...
@pytest.mark.asyncio
async def test_creer_rdv_detects_legacy_praticien_conflict_when_end_is_null(db, medecin, patient, acte):
    debut = datetime(2026, 7, 20, 10, 0)
    rdv, _ = await creer_rdv(
        patient_id=patient.id, praticien_id=medecin.id, acte_id=acte.id,
        date_heure=debut, salle=None, db=db, clinic_id=1,
    )
    rdv.date_heure_fin = None
    await db.flush()

    with pytest.raises(ValueError, match="[Cc]onflit"):
        await creer_rdv(
            patient_id=patient.id, praticien_id=medecin.id, acte_id=acte.id,
            date_heure=debut + timedelta(minutes=5), salle=None, db=db, clinic_id=1,
        )


@pytest.mark.asyncio
async def test_creer_rdv_detects_legacy_salle_conflict_when_end_is_null(db, medecin, assistante, patient, acte):
    ...
    with pytest.raises(ValueError, match="[Ss]alle"):
        await creer_rdv(
            patient_id=patient.id, praticien_id=assistante.id, acte_id=acte.id,
