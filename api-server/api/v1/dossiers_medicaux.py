...
async def create_patient_dossier(
    ...
    current_user=Depends(require_role(RoleEnum.MEDECIN, RoleEnum.ESTHETICIENNE, RoleEnum.ASSISTANTE)),
):
    """Enregistre le dossier, éventuellement comme brouillon d'accueil."""
    try:
        dossier = await create_dossier(
            ...
            draft=current_user.get("role") == RoleEnum.ASSISTANTE.value,
        )
...
async def get_dossier_detail(
    ...
    sensitive_fields_hidden = role in {"directrice", "estheticienne"}
    ...
    return {
        ...
        "observations": (
            ...
            if dossier.observations_enc and not sensitive_fields_hidden
            else "[ACCÈS RÉSERVÉ]"
        ),
        ...
        "effets_secondaires": (
            ...
            if not sensitive_fields_hidden
            else "[ACCÈS RÉSERVÉ]"
        ),
...
async def create_consentement(
    ...
    try:
        ...
        return {
            ...
            "episode_id": consent.episode_id,
            "intervention_id": consent.intervention_id,
        }
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))


@router.get("/{patient_id}/consentements/{consentement_id}/view")
async def view_consentement(
    patient_id: int,
    consentement_id: int,
    request: Request,
    db: AsyncSession = Depends(get_db),
    ...
    """Ouvre le PDF signé d'un consentement, avec contrôle patient/clinique."""
    result = await db.execute(
    ...
    pdf_path = Path(settings.data_dir) / "uploads" / f"consentement_{consent.id}.pdf"
    if not pdf_path.is_file():
        raise HTTPException(status_code=404, detail="Document signé indisponible")

    await log_access(
        db=db, utilisateur_id=current_user["id"], patient_id=patient_id,
        action="VIEW_CONSENTEMENT", resource_type="consentement", resource_id=consent.id,
        ip_address=request.client.host if request.client else None,
    )
    return FileResponse(
        path=pdf_path,
        media_type="application/pdf",
        filename=f"consentement_{consent.id}.pdf",
    ...
    return [
        {
            ...
            "document_url": f"/api/private/patients/{patient_id}/consentements/{c.id}/view",
        }
...
async def list_photos(
    ...
    if episode_id is not None:
        query = query.where(PhotoClinic.episode_id == episode_id)
    if intervention_id is not None:
        query = query.where(PhotoClinic.intervention_id == intervention_id)
