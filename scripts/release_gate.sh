...
printf '%s\n' '[16/18] Frontière refresh token'
  if grep -RInE 'localStorage\.(getItem|setItem|removeItem)\([^)]*(access_token|refresh_token)|refresh_token.*localStorage|localStorage.*refresh_token' autocommerce-app/client/src --exclude-dir=node_modules --exclude-dir=__tests__ --exclude='*.test.*' --exclude='*.spec.*'; then
