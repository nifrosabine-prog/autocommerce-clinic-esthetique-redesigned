...
from models.security import MfaChallenge
...
async def mfa_setup(
    ...
    try:
        ...
        branding_context = await get_branding_context(db, clinic_id=current_user["clinic_id"])
    except Exception:
        pass
...
async def mfa_confirm(
    ...
    # Vérifier le code OTP sur le secret déchiffré.
    mfa_secret = decrypt_mfa_secret(user.mfa_secret)
    if not verify_totp(mfa_secret, payload.otp):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Code OTP invalide. Vérifiez votre application d'authentification."
        )

    # Activer le MFA et migrer un éventuel secret historique brut.
...
async def mfa_verify(
    ...
    if challenge.get("type") != "mfa_challenge" or not challenge.get("jti"):
    ...
    challenge_row = await db.scalar(
        select(MfaChallenge)
        .where(
            MfaChallenge.utilisateur_id == user_id,
            MfaChallenge.jti == challenge_jti,
    ...
    if not challenge_row:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Challenge MFA invalide ou déjà utilisé")
    ...
    # Vérifier le code OTP sur le secret déchiffré.
    mfa_secret = decrypt_mfa_secret(user.mfa_secret)
    otp_valid = verify_totp(mfa_secret, payload.otp)
    ...
    # Réinitialiser les tentatives et rendre le challenge mono-usage.
    challenge_row.consumed_at = datetime.utcnow()
    # Les installations v2 peuvent encore avoir un secret brut : le migrer
    # automatiquement au premier flux MFA réussi.
    user.mfa_secret = encrypt_mfa_secret(mfa_secret)
    user.mfa_failed_attempts = 0
    user.mfa_locked_until = None
