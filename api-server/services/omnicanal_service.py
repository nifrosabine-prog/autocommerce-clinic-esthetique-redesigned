...
from models.database import Patient, Utilisateur
...
async def close_conversation(conversation_id: int, db: AsyncSession, *, clinic_id: int) -> Conversation:
...
async def assign_conversation(
    ...
    *,
    ...
    """Assigne une conversation et un utilisateur du même tenant."""
    result = await db.execute(
        select(Conversation).where(
            Conversation.id == conversation_id,
            Conversation.clinic_id == clinic_id,
    ...
    if not conversation:
        raise ValueError("Conversation non trouvée")
    assignee = await db.scalar(select(Utilisateur).where(
        Utilisateur.id == assignee_id,
        Utilisateur.clinic_id == clinic_id,
        Utilisateur.is_active,
    ))
    if not assignee:
        raise ValueError("Utilisateur assigné non trouvé dans cette clinique")
...
async def add_tags_to_conversation(
    ...
    *,
    ...
    """Ajoute des tags à une conversation du tenant demandé."""
    result = await db.execute(
        select(Conversation).where(
            Conversation.id == conversation_id,
            Conversation.clinic_id == clinic_id,
    ...
    if canal == "whatsapp":
        result = await db.execute(
            select(Patient).where(
                Patient.whatsapp_phone == contact_external_id,
...
async def retry_failed_message(
    ...
    *,
    ...
    """Tente de renvoyer un message en échec du tenant demandé."""
    result = await db.execute(
        select(MessageOmnicanal).where(
            MessageOmnicanal.id == message_id,
            MessageOmnicanal.clinic_id == clinic_id,
        )
    )
    message = result.scalar_one_or_none()
...
async def get_channel_stats(
    db: AsyncSession, clinic_id: Optional[int] = None
) -> dict:
    """Retourne le statut des canaux du tenant courant, sans fallback HTTP implicite."""
    from services.access_control import clinic_id_for

    clinic_id = clinic_id_for(
        {"clinic_id": clinic_id} if clinic_id is not None else None
    )
