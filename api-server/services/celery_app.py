...
def reminder_j1_task():
    ...
    async def run():
        ...
        async with session_factory() as db:
            ...
            rows = await db.execute(select(Utilisateur.clinic_id).where(
                Utilisateur.is_active.is_(True)
            ).distinct())
            for (clinic_id,) in rows.all():
                await reminder_j1(db, clinic_id=clinic_id)
            await db.commit()
...
def reminder_h2_task():
    ...
    async def run():
        ...
        async with session_factory() as db:
            ...
            for (clinic_id,) in rows.all():
                await reminder_h2(db, clinic_id=clinic_id)
            await db.commit()
...
def anniversaire_task():
    ...
    async def run():
        ...
        async with session_factory() as db:
            ...
            for (clinic_id,) in rows.all():
                await check_anniversaire(db, clinic_id=clinic_id)
            await db.commit()
...
def inactivite_task():
    ...
    async def run():
        ...
        async with session_factory() as db:
            ...
            for (clinic_id,) in rows.all():
                await check_inactivite(db, clinic_id=clinic_id)
            await db.commit()
...
def expire_points_task():
    ...
    async def run():
        ...
        async with session_factory() as db:
            ...
            for (clinic_id,) in rows.all():
                await expire_points(db, clinic_id=clinic_id)
            await db.commit()
    ...
    async def run():
        ...
        async with session_factory() as db:
            ...
            for commercial in commerciaux:
                await calculer_commissions_mois(
                    commercial.id, mois_precedent, db,
                    clinic_id=commercial.clinic_id,
                )
            await db.commit()
