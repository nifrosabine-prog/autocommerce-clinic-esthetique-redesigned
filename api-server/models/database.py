...
class Utilisateur(Base):
    ...
    diplomes: Mapped[Optional[list]] = mapped_column(JSON, nullable=True)
    certifications: Mapped[Optional[list]] = mapped_column(JSON, nullable=True)
    documents_professionnels: Mapped[Optional[list]] = mapped_column(JSON, nullable=True)
    ...
    # Un praticien actif reste interne par défaut ; la landing ne publie que
    # les profils explicitement validés par la direction.
    is_public: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    # Relations
    actes_pratiques: Mapped[List["ActeMedical"]] = relationship(
        "ActeMedical", secondary="utilisateurs_actes", back_populates="praticiens"
    )
    last_login: Mapped[Optional[datetime]] = mapped_column(DateTime, nullable=True)
    # ── MFA / 2FA ───────────────────────────────────────────
    # Un secret TOTP Fernet chiffré dépasse 100 caractères ; Text évite toute troncature.
...
class ActeMedical(Base):
    ...
    is_active: Mapped[bool] = mapped_column(Boolean, default=True)
    # Publication distincte de l'état interne : un acte peut être actif dans
    # le catalogue clinique sans être proposé à la réservation publique.
    is_public: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)

    __table_args__ = (
        CheckConstraint("length(trim(nom)) >= 2", name="ck_actes_nom_non_vide"),
        CheckConstraint("length(trim(categorie)) >= 2", name="ck_actes_categorie_non_vide"),
        CheckConstraint("duree_minutes BETWEEN 5 AND 480", name="ck_actes_duree_bornee"),
        CheckConstraint("prix_base >= 0", name="ck_actes_prix_non_negatif"),
        CheckConstraint("is_gratuit = 1 OR prix_base > 0", name="ck_actes_prix_ou_gratuit"),
        UniqueConstraint("clinic_id", "nom_normalise", name="uq_actes_clinic_nom_normalise"),
        Index("ix_actes_publics", "clinic_id", "is_active", "is_public"),
    )
...
class Salle(Base):
    ...
    clinic_id: Mapped[int] = mapped_column(Integer, default=1, nullable=False)
    nom: Mapped[str] = mapped_column(String(100), nullable=False)
    type: Mapped[str] = mapped_column(String(50), nullable=False)  # consultation, operation, repos
    description: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    is_active: Mapped[bool] = mapped_column(Boolean, default=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)

    # Relations
    rdvs: Mapped[List["RendezVous"]] = relationship("RendezVous", back_populates="salle_rel")


class UtilisateurActe(Base):
    __tablename__ = "utilisateurs_actes"

    utilisateur_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("utilisateurs.id", ondelete="CASCADE"), primary_key=True
    )
    acte_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("actes_medicaux.id", ondelete="CASCADE"), primary_key=True
    )
...
class DossierMedical(Base):
    ...
    # Un brouillon est persistant mais non clôturé : il n’entraîne jamais de
    # facturation et la clôture reste conditionnée au consentement de l’acte.
    statut_clinique: Mapped[str] = mapped_column(String(20), default="cloture", nullable=False)
...
class Consentement(Base):
    ...
    praticien_signataire_id: Mapped[Optional[int]] = mapped_column(
        Integer, ForeignKey("utilisateurs.id", ondelete="SET NULL"), nullable=True
    )
    ...
    contrat_snapshot: Mapped[Optional[dict]] = mapped_column(JSON, nullable=True)
    attestation_praticien: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    signature_praticien_base64: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
...
class Facture(Base):
    ...
    dossier: Mapped[Optional["DossierMedical"]] = relationship("DossierMedical", back_populates="facture")
    createur: Mapped[Optional["Utilisateur"]] = relationship(
...
class ClinicSubscription(Base):
    """Abonnement et état de facturation d’une clinique supervisée globalement."""
    __tablename__ = "clinic_subscriptions"

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    clinic_id: Mapped[int] = mapped_column(Integer, nullable=False, unique=True, index=True)
    clinic_name: Mapped[str] = mapped_column(String(200), nullable=False)
    plan: Mapped[str] = mapped_column(String(50), nullable=False, default="essentiel")
    status: Mapped[str] = mapped_column(String(30), nullable=False, default="active")
    started_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, nullable=False)
    expires_at: Mapped[Optional[datetime]] = mapped_column(DateTime, nullable=True)
    monthly_amount: Mapped[Decimal] = mapped_column(Numeric(10, 3), default=Decimal("0.000"), nullable=False)
    ...
    __table_args__ = (
        ...
        Index("ix_clinic_subscriptions_expires_at", "expires_at"),
    )
...
def normalize_async_database_url(database_url: str) -> str:
    """Normalise une URL de base de données pour SQLAlchemy asyncio.

    Railway fournit PostgreSQL sous la forme ``postgresql://``. Le moteur
    SQLAlchemy asynchrone doit utiliser explicitement le dialecte ``asyncpg``.
    """
...
class Pointage(Base):
    """Enregistrement des heures de présence des praticiens."""
    ...
    debut: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, nullable=False)
...
# ── Clinic Operations v2 ───────────────────────────────────────────────────

class CureTraitement(Base):
    __tablename__ = "cures_traitements"

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    clinic_id: Mapped[int] = mapped_column(Integer, nullable=False, index=True)
    patient_id: Mapped[int] = mapped_column(Integer, ForeignKey("patients.id", ondelete="RESTRICT"), nullable=False, index=True)
    acte_id: Mapped[Optional[int]] = mapped_column(Integer, ForeignKey("actes_medicaux.id", ondelete="SET NULL"), nullable=True)
    nom: Mapped[str] = mapped_column(String(200), nullable=False)
    description: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    zone_anatomique: Mapped[Optional[str]] = mapped_column(String(100), nullable=True)
    seances_prevues: Mapped[int] = mapped_column(Integer, nullable=False, default=1)
    statut: Mapped[str] = mapped_column(String(30), nullable=False, default="active", index=True)
    prochaine_seance_at: Mapped[Optional[datetime]] = mapped_column(DateTime, nullable=True)
    notes: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    ...
    __table_args__ = (
        CheckConstraint("seances_prevues > 0", name="ck_cure_seances_prevues_positive"),
        Index("ix_cures_clinic_status", "clinic_id", "statut"),
    )


class SeanceCure(Base):
    __tablename__ = "seances_cures"

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    clinic_id: Mapped[int] = mapped_column(Integer, nullable=False, index=True)
    cure_id: Mapped[int] = mapped_column(Integer, ForeignKey("cures_traitements.id", ondelete="CASCADE"), nullable=False, index=True)
    numero: Mapped[int] = mapped_column(Integer, nullable=False)
    rendez_vous_id: Mapped[Optional[int]] = mapped_column(Integer, ForeignKey("rendez_vous.id", ondelete="SET NULL"), nullable=True)
    dossier_id: Mapped[Optional[int]] = mapped_column(Integer, ForeignKey("dossiers_medicaux.id", ondelete="SET NULL"), nullable=True)
    praticien_id: Mapped[Optional[int]] = mapped_column(Integer, ForeignKey("utilisateurs.id", ondelete="SET NULL"), nullable=True)
    planifiee_at: Mapped[Optional[datetime]] = mapped_column(DateTime, nullable=True)
    realisee_at: Mapped[Optional[datetime]] = mapped_column(DateTime, nullable=True)
    statut: Mapped[str] = mapped_column(String(30), nullable=False, default="a_venir", index=True)
    zone_anatomique: Mapped[Optional[str]] = mapped_column(String(100), nullable=True)
    produits_lots: Mapped[Optional[list]] = mapped_column(JSON, nullable=True)
    photos_ids: Mapped[Optional[list]] = mapped_column(JSON, nullable=True)
    notes: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    suivi_recommande_le: Mapped[Optional[date]] = mapped_column(Date, nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, nullable=False)

    __table_args__ = (
        UniqueConstraint("clinic_id", "cure_id", "numero", name="uq_seance_cure_numero"),
        Index("ix_seances_cures_clinic_status", "clinic_id", "statut"),
    )


class SuiviPostActe(Base):
    __tablename__ = "suivis_post_acte"
    ...
    statut: Mapped[str] = mapped_column(String(30), nullable=False, default="a_faire", index=True)
    assigne_a_id: Mapped[Optional[int]] = mapped_column(Integer, ForeignKey("utilisateurs.id", ondelete="SET NULL"), nullable=True)
    notes: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    termine_at: Mapped[Optional[datetime]] = mapped_column(DateTime, nullable=True)
    created_by: Mapped[Optional[int]] = mapped_column(Integer, ForeignKey("utilisateurs.id", ondelete="SET NULL"), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, nullable=False)
    updated_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow, nullable=False)
...
class EvenementIndesirable(Base):
    __tablename__ = "evenements_indesirables"
    ...
    acte_id: Mapped[Optional[int]] = mapped_column(Integer, ForeignKey("actes_medicaux.id", ondelete="SET NULL"), nullable=True)
    seance_id: Mapped[Optional[int]] = mapped_column(Integer, ForeignKey("seances_cures.id", ondelete="SET NULL"), nullable=True)
    survenu_at: Mapped[datetime] = mapped_column(DateTime, nullable=False, index=True)
    zone_anatomique: Mapped[Optional[str]] = mapped_column(String(100), nullable=True)
    description: Mapped[str] = mapped_column(Text, nullable=False)
    gravite: Mapped[str] = mapped_column(String(20), nullable=False, default="faible", index=True)
    action_effectuee: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    praticien_informe: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
    suivi: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    statut: Mapped[str] = mapped_column(String(30), nullable=False, default="ouvert", index=True)
    created_by: Mapped[int] = mapped_column(Integer, ForeignKey("utilisateurs.id", ondelete="RESTRICT"), nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, nullable=False)
    updated_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow, nullable=False)

    __table_args__ = (
        CheckConstraint("gravite in ('faible', 'moderee', 'elevee', 'critique')", name="ck_evenement_gravite"),
        Index("ix_evenements_clinic_status", "clinic_id", "statut"),
    )


class ProtocoleSoin(Base):
    ...
    acte_id: Mapped[Optional[int]] = mapped_column(Integer, ForeignKey("actes_medicaux.id", ondelete="SET NULL"), nullable=True)
    nom: Mapped[str] = mapped_column(String(200), nullable=False)
    categorie: Mapped[str] = mapped_column(String(80), nullable=False, default="esthetique")
    version: Mapped[str] = mapped_column(String(30), nullable=False, default="1.0")
    etapes_avant: Mapped[list] = mapped_column(JSON, nullable=False, default=list)
    etapes_pendant: Mapped[list] = mapped_column(JSON, nullable=False, default=list)
    etapes_apres: Mapped[list] = mapped_column(JSON, nullable=False, default=list)
    ...
    __table_args__ = (
        UniqueConstraint("clinic_id", "nom", "version", name="uq_protocole_clinic_nom_version"),
    )
