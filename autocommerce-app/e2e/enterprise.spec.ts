...
async function loginThroughUi(page: Page, email: string, password: string): Promise<string> {
  ...
  if (payload.mfa_required) {
    ...
    const verifyResponse = page.waitForResponse((verify) => (
      verify.url().includes('/auth/mfa/verify') && verify.request().method() === 'POST'
    ));
...
test('MFA setup/confirm/verify is exercised when staging account is not already enabled', async ({ page }) => {
  ...
  let mfaLoginResponse: Response | undefined;
  for (let attempt = 0; attempt < 2; attempt += 1) {
    await page.goto('/login');
    ...
    await page.getByRole('button', { name: 'Se connecter' }).click();
    const candidate = await mfaLogin;
    if (candidate.status() === 429 && attempt === 0) {
      const retryAfter = Number(candidate.headers()['retry-after'] || '60');
      await page.waitForTimeout((Math.max(1, retryAfter) + 1) * 1000);
      continue;
    }
    mfaLoginResponse = candidate;
    break;
  }
  if (!mfaLoginResponse) throw new Error('MFA login rate limit did not clear within the E2E retry window');
  ...
  const verifyResponse = page.waitForResponse((response) => (
    response.url().includes('/auth/mfa/verify') && response.request().method() === 'POST'
  ));
