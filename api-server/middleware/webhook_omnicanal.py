...
def _resolve_webhook_clinic_id(settings, clinic_id: Optional[int]) -> int:
    """Résout le tenant du webhook sans jamais inventer une clinique en production."""
    if clinic_id is not None and int(clinic_id) > 0:
        return int(clinic_id)
    if settings.social_webhook_clinic_id and int(settings.social_webhook_clinic_id) > 0:
        return int(settings.social_webhook_clinic_id)
    if settings.env in {"test", "development"}:
        return int(settings.clinic_id or 1)
    raise RuntimeError("SOCIAL_WEBHOOK_CLINIC_ID obligatoire pour les webhooks en production")
...
async def process_incoming_webhook(
    ...
    path_canal: Optional[str] = None,
) -> dict:
    """Traite un webhook entrant multi-canal dans un tenant explicite."""
    ...
    try:
        clinic_id = _resolve_webhook_clinic_id(settings, clinic_id)
    except RuntimeError as exc:
        logger.error("Webhook rejected: %s", exc)
        return {"error": "Webhook non configuré: mapping clinique manquant", "success": False}
