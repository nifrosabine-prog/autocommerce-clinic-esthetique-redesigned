...
class TestOmnicanalService:
    ...
    async def test_get_or_create_conversation_returns_existing(self, db):
        ...
        conv1 = await get_or_create_conversation(
            canal="whatsapp", contact_external_id="+21699999998", clinic_id=1, db=db,
        )
    ...
    async def test_receive_message_creates_in_new_conversation(self, db):
        ...
        result = await receive_message(
            canal="whatsapp", contact_external_id="+21699999997", content="Bonjour, j'ai une question",
            clinic_id=1, db=db,
        )
    ...
    @pytest.mark.asyncio
    async def test_receive_message_does_not_match_patient_from_other_clinic(self, db, patient):
        from services.omnicanal_service import receive_message
        patient.clinic_id = 2
        await db.flush()
        result = await receive_message(
            canal="whatsapp",
            contact_external_id=patient.whatsapp_phone,
            content="Message provenant d'un autre tenant",
            clinic_id=1,
            db=db,
        )
        assert result["patient_matched"] is False
        assert result["conversation"].patient_id is None
    ...
    async def test_send_reply_via_dev_mode(self, db):
        ...
        recv = await receive_message(
            ...
            content="Question", clinic_id=1, db=db,
        )
    ...
    async def test_close_conversation(self, db):
        ...
        conv = await get_or_create_conversation(
            canal="whatsapp", contact_external_id="+21699999995", clinic_id=1, db=db,
        )
        closed = await close_conversation(conv.id, db, clinic_id=1)
        assert closed.statut == "fermee"

    @pytest.mark.asyncio
    async def test_assign_conversation(self, db, medecin):
        from services.omnicanal_service import get_or_create_conversation, assign_conversation
        conv = await get_or_create_conversation(
            canal="whatsapp", contact_external_id="+21699999994", clinic_id=1, db=db,
        )
        assigned = await assign_conversation(conv.id, medecin.id, db, clinic_id=1)
        assert assigned.assignee_id == medecin.id
    ...
    async def test_add_tags_to_conversation(self, db):
        ...
        conv = await get_or_create_conversation(
            canal="whatsapp", contact_external_id="+21699999993", clinic_id=1, db=db,
        )
        tagged = await add_tags_to_conversation(conv.id, ["vip", "relance"], db, clinic_id=1)
    ...
    async def test_add_tags_no_duplicates(self, db):
        from services.omnicanal_service import get_or_create_conversation, add_tags_to_conversation
        conv = await get_or_create_conversation(
            canal="whatsapp", contact_external_id="+21699999992", clinic_id=1, db=db,
        )
        await add_tags_to_conversation(conv.id, ["vip"], db, clinic_id=1)
        tagged = await add_tags_to_conversation(conv.id, ["vip", "nouveau"], db, clinic_id=1)
        tags = json.loads(tagged.tags)
