...
from models.database import UtilisationLot, LotInjectable, Patient
...
async def process_injection_reminders(
    db: AsyncSession,
    ...
    stmt = (
        ...
        .join(Patient, UtilisationLot.patient_id == Patient.id)
        .options(
...
async def send_injection_reminder(utilisation: UtilisationLot, db: AsyncSession) -> bool:
    ...
    if not patient.whatsapp_phone or patient.opted_out:
