...
# Les secrets ne sont jamais embarqués dans le ZIP. Le déployeur fournit
# un fichier externe compatible avec le contrat des compose.
ENV_FILE="${DEPLOY_ENV_FILE:-.env}"
API_ENV_FILE="${API_ENV_FILE:-$ENV_FILE}"

for required_file in "$ENV_FILE" "api-server/scripts/pre_deploy_security_check.py" "scripts/verify_infrastructure.sh"; do
...
API_ENV_FILE="$API_ENV_FILE" ./scripts/verify_infrastructure.sh
...
ADMIN_EMAIL="$ADMIN_EMAIL" ADMIN_PASSWORD="$ADMIN_PASSWORD" \
  docker compose --env-file "$ENV_FILE" -f "$COMPOSE_FILE" run --rm -T api python api-server/scripts/seed_admin.py
