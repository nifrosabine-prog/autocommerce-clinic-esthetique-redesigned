...
Pour une recette isolée ou une production, utiliser `.env.example` comme contrat puis fournir un fichier `.env` externe à Compose. Les compose staging et mono-vps transmettent désormais explicitement ce fichier à l’API, au worker et au beat ; aucun fichier de secrets n’est embarqué dans le ZIP. Les environnements doivent avoir des secrets, bases, volumes et sauvegardes distincts.
...
cp .env.example .env
chmod 600 .env
# Remplacer les valeurs CHANGE_ME par les secrets, domaines et chemins TLS réels.
```
...
```bash
API_ENV_FILE=.env ./scripts/verify_infrastructure.sh
DEPLOY_ENV_FILE=.env ./scripts/first_deploy.sh
```
