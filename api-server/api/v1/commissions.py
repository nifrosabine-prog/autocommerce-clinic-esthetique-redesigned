...
async def _noms_utilisateurs(db: AsyncSession, ids: set[int], clinic_id: int) -> dict[int, str]:
    ids = {i for i in ids if i}
    if not ids:
        return {}
    result = await db.execute(select(Utilisateur).where(
...
async def total_du_route(
    ...
    return {"commercial_id": commercial_id, "total_du": await total_du_par_commercial(
        commercial_id, db, clinic_id=current_user["clinic_id"]
    )}
...
async def valider_route(
    ...
    try:
        commission = await valider_commission(
            commission_id, current_user["id"], db,
...
async def payer_route(
    ...
    try:
        commission = await marquer_payee(
            commission_id, payload.date_paiement, db,
            clinic_id=current_user["clinic_id"],
        )
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
