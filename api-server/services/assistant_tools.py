...
async def _praticien_id_for(current_user: Dict[str, Any], db: AsyncSession) -> Optional[int]:
    ...
    """
    role = str(current_user.get("role", "")).replace("RoleEnum.", "").lower()
    if role in ("medecin", "estheticienne", "directrice"):
        return current_user.get("id")
    if role == "assistante":
        # Premier médecin actif du tenant courant.
        res = await db.execute(
            select(Utilisateur).where(
                Utilisateur.role == "medecin",
                Utilisateur.clinic_id == current_user.get("clinic_id"),
    ...
    res = await db.execute(
        select(func.count(RendezVous.id)).where(
            and_(
                ...
                RendezVous.clinic_id == current_user.get("clinic_id"),
...
async def get_next_rdv(current_user: Dict[str, Any], db: AsyncSession) -> Dict[str, Any]:
    ...
    res = await db.execute(
        ...
        .where(RendezVous.clinic_id == current_user.get("clinic_id"))
        .where(Patient.clinic_id == current_user.get("clinic_id"))
...
async def list_inactive_patients(
    ...
    res = await db.execute(
        select(Patient)
        .where(
            Patient.clinic_id == current_user.get("clinic_id"),
...
async def get_stock_overview(
    ...
    dashboard = await get_stock_dashboard(db, clinic_id=current_user.get("clinic_id"))
    if produit_nom:
        produits = [
