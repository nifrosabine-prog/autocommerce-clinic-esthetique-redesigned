...
class Settings(BaseSettings):
    ...
    # Clé distincte pour chiffrer les secrets TOTP au repos.
    ...
    webhook_max_body_size: int = 1_000_000
    ...
    # Optionnel : modèles autorisés séparés par virgules, sous la forme
    # `provider:model` ou `model`. Vide = modèle configuré uniquement.
    llm_model_allowlist: str = ""
    openai_api_key: str = ""
    # Base URL configurable pour les environnements compatibles OpenAI ; le défaut reste l’API officielle.
...
def _validate_production_secrets(settings: "Settings") -> None:
    ...
    if _contains_placeholder(settings.mfa_encryption_key):
        erreurs.append("MFA_ENCRYPTION_KEY contient un placeholder interdit")
