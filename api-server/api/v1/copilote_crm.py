...
async def summarize_patient_file(
    ...
    """
    try:
        summary = await CopiloteCRMService.summarize_patient_file(
            session, patient_id, clinic_id=current_user["clinic_id"]
        )
        if "error" in summary:
            raise HTTPException(
...
async def suggest_treatment(
    ...
    """
    try:
        suggestions = await CopiloteCRMService.suggest_treatment(
            session, patient_id, clinic_id=current_user["clinic_id"]
        )
...
async def generate_medical_report(
    ...
    """
    try:
        report = await CopiloteCRMService.generate_medical_report(
            session, patient_id, clinic_id=current_user["clinic_id"]
        )
        if "error" in report:
            raise HTTPException(
...
async def prepare_next_appointment(
    ...
    """
    try:
        preparation = await CopiloteCRMService.prepare_next_appointment(
            session, patient_id, clinic_id=current_user["clinic_id"]
        )
