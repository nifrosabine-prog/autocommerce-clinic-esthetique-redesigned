...
def test_mfa_challenge_cannot_be_replayed(client, user_with_mfa):
    import pyotp
    user, secret = user_with_mfa
    login = client.post("/api/v1/auth/login", json={"email": user.email, "password": "MotDePasse!23"})
    challenge_token = login.json()["challenge_token"]
    otp = pyotp.TOTP(secret).now()
    first = client.post("/api/v1/auth/mfa/verify", json={"challenge_token": challenge_token, "otp": otp})
    assert first.status_code == 200
    replay = client.post("/api/v1/auth/mfa/verify", json={"challenge_token": challenge_token, "otp": otp})
    assert replay.status_code == 401
    assert "déjà utilisé" in replay.json()["detail"]
