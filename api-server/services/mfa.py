...
def _mfa_fernet() -> Fernet:
    key = getattr(settings, "mfa_encryption_key", "") or getattr(settings, "fernet_key", "")
    if not key:
        raise ValueError("MFA_ENCRYPTION_KEY ou FERNET_KEY non configurée")
...
def decrypt_mfa_secret(stored: str) -> str:
    """Déchiffre un secret TOTP; accepte temporairement l’ancien format brut."""
    if not stored:
        return ""
    try:
        return _mfa_fernet().decrypt(stored.encode()).decode()
    except (InvalidToken, ValueError):
        # Compatibilité contrôlée avec les installations v2. Le secret est
        # re-chiffré dès qu’un flux MFA authentifié le réutilise avec succès.
        logger.warning("Legacy plaintext MFA secret encountered; re-encryption required")
        return stored
