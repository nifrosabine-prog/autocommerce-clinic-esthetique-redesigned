...
async def list_pending_billing(
    ...
    results = []
    for d, p in rows:
        existing = await find_active_facture_for_dossier(db, d, current_user["clinic_id"])
        if existing:
            # Répare les dossiers historiques créés avant dossier_id.
            d.statut_facturation = "facture"
            if existing.dossier_id != d.id:
...
async def generate_invoice_ia(
    ...
    from services.factures import find_active_facture_for_dossier
    existing = await find_active_facture_for_dossier(db, dossier, current_user["clinic_id"])
    if existing:
        dossier.statut_facturation = "facture"
        if existing.dossier_id != dossier.id:
            existing.dossier_id = dossier.id
        raise HTTPException(
            status_code=409,
            detail=f"Ce dossier est déjà facturé par {existing.numero_facture}",
        )
