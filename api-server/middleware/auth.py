...
def create_mfa_challenge_token(user_id: int) -> str:
    return create_access_token(
        {"sub": str(user_id), "type": "mfa_challenge", "jti": uuid4().hex},
...
async def authenticate_user(identifier: str, password: str, db: AsyncSession) -> Optional[Utilisateur]:
    """Authentifie via un identifiant générique stocké dans le champ email historique."""
    normalized_identifier = identifier.strip().lower()
    result = await db.execute(select(Utilisateur).where(Utilisateur.email == normalized_identifier))
