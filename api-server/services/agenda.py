...
def _utc_naive(value: datetime) -> datetime:
    """Convertit une date aware ou naive en UTC sans tzinfo pour PostgreSQL."""
...
async def creer_rdv(
    ...
    """
    clinic_id = _resolve_clinic_id(clinic_id)
    date_heure = _utc_naive(date_heure)
    ...
    conflit_praticien = await db.execute(
        ...
        .where(
            and_(
                ...
                or_(
                    RendezVous.date_heure_fin.is_(None),
    ...
    # Vérifier que les ressources appartiennent bien à la clinique courante.
    ...
    praticien_result = await db.execute(select(Utilisateur).where(
...
async def reminder_j1(
    ...
    result = await db.execute(
        ...
        .where(
            ...
            Patient.clinic_id == clinic_id,
            Utilisateur.clinic_id == clinic_id,
            RendezVous.date_heure_debut >= debut,
        )
...
async def reminder_h2(
    ...
    result = await db.execute(
        ...
        .where(
            RendezVous.clinic_id == clinic_id,
            Patient.clinic_id == clinic_id,
            RendezVous.date_heure_debut >= dans_2h - marge,
        )
