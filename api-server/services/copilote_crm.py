...
class CopiloteCRMService:
    ...
    async def summarize_patient_file(
        ...
        """
        patient = await CopiloteCRMService._load_patient_full(
            session, patient_id, clinic_id=clinic_id
        )
        if not patient:
    ...
    async def suggest_treatment(
        ...
        summary = await CopiloteCRMService.summarize_patient_file(
            session, patient_id, settings, llm, clinic_id=clinic_id
        )
        suggestion = {
            "patient_id": patient_id,
